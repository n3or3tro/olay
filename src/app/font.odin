...
Icon_Wave 			:: rune(0xE009)
Icon_Record 		:: rune(0xE010)
Icon_Keyboard		:: rune(0xE011)
Icon_Sort_Alpha_Up  :: rune(0xE012)
Icon_Sort_Alpha_Down:: rune(0xE013)
Icon_Add_Folder		:: rune(0xE014)
...
font_add_shaped_run :: proc(
	...
	arena, scratch := arena_allocator_new("font-add-shaped-rune")
	...
	// Store raw HarfBuzz design-unit positions. The conversion to pixel space
	// happens later in font_get_render_info where the correct font_size is known.
	for &glyph, i in temp_glyph_buffer {
		x_i32, y_i32 := kb.PositionGlyph(cursor, &glyph)
		my_glyph := Glyph {
			glyph = glyph,
			pos   = {f32(x_i32), f32(y_i32)},
		}
...
font_get_render_info :: proc(
	...
	// Set pixel size once up front so face.size.metrics has the correct scale factors
	// for converting design-unit positions to pixel positions.
	sz_err := ft.set_pixel_sizes(face, 0, font_size)
	assert(sz_err == .Ok)
	x_scale := i64(face.size.metrics.x_scale)
	y_scale := i64(face.size.metrics.y_scale)
	...
	for glyph, i in glyph_buffer {
		// Convert raw design-unit positions to pixel space.
		scaled_pos := Vec2_f32{
			f32((i64(glyph.pos.x) * x_scale) >> 16) / 64.0,
			f32((i64(glyph.pos.y) * y_scale) >> 16) / 64.0,
		}
