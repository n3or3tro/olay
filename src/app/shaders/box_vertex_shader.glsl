...
layout(location = 11) in uint ui_flags; 	// 0 = normal quad, 1 = text, 2 = to be decided    
...
layout(location = 8)  flat out uint out_ui_flags; // 0 = normal quad, 1 = text, 2 = to be decided    
...
flat out vec4 inst_tl_color;
flat out vec4 inst_tr_color;
flat out vec4 inst_bl_color;
flat out vec4 inst_br_color;
...
void main() {
	...
	if (flag_set(ui_flags, UI_Flag_Font)) { 
		...
		// similar to audio spectrum, we repurpose texture_top_left to pass in
		// relevant knob data, since knobs aren't textured.
		out_texture_uv = texture_top_left;
	}
	...
	out_ui_flags = ui_flags;
	out_font_size = font_size;
	out_clip_tl = clip_tl;
	out_clip_br = clip_br;
	out_rotation_radians = rotation_radians;

	inst_tl_color = tl_color;
	inst_tr_color = tr_color;
	inst_bl_color = bl_color;
	inst_br_color = br_color;
