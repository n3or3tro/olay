...
file_dialog :: proc(multiselect: bool) -> ([dynamic]cstring, bool) {
	...
	// 	return file_dialog_macos(multiselect)
	// }
}
...
// when ODIN_OS == .Darwin {
//     file_dialog_macos :: proc(multiselect: bool = false) -> ([dynamic]cstring, bool) {
//         paths := make([dynamic]cstring)

//         panel := NS.OpenPanel.openPanel()
//         NS.OpenPanel.setAllowsMultipleSelection(panel, multiselect)
//         NS.OpenPanel.setCanChooseFiles(panel, true)
//         NS.OpenPanel.setCanChooseDirectories(panel, false)
//         NS.OpenPanel.setResolvesAliases(panel, true)

//         response := NS.SavePanel.runModal(panel)
//         if response != .OK {
//             return paths, false
//         }

//         urls := NS.OpenPanel.URLs(panel)
//         count := NS.Array.count(urls)
//         for i in 0 ..< count {
//             url := NS.Array.objectAs(urls, i, ^NS.URL)
//             path_ns := NS.URL.relativePath(url)
//             path_odin := NS.String.odinString(path_ns)
//             append(&paths, s.clone_to_cstring(path_odin))
//         }
