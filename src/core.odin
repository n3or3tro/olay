...
box_from_cache :: proc(id_string: string, flags: Box_Flags, config: Box_Config) -> ^Box {
	...
	// 	return ui_state.box_cache[id_string]
	// } else {
	// 	persistant_id_string := s.clone(id_string)
	// 	new_box := box_make(id_string, flags, style)
	// 	ui_state.box_cache[persistant_id_string] = new_box
	// 	return new_box
	// }
	// return box_make(id_string, flags, style)
	...
	if cached_box, ok := ui_state.box_cache[id_string]; ok {
		...
		// 1. Update the box's configuration from the current frame's builder code.
		box.flags = flags
		box.config = config

		// 2. Reset the CALCULATED layout state from the previous frame.
		if box.config.semantic_size.x.type != .Fixed {box.width = 0}
		if box.config.semantic_size.y.type != .Fixed {box.height = 0}

		// 3. Re-apply fixed sizes from the new config.
		...
		is_new = true
		persistant_id_string := s.clone(id_string)
		// The original box_make will be called, but it WON'T parent the box.
